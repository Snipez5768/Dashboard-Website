...
function alsTermin(e, quelle) {
  ...
  return {
    ...
    time: zeit,
    ort: e.location || "",
    /* Bei ganztägigen Einträgen nennt Google den Tag NACH dem
       letzten. Für Ferienblöcke wird das gebraucht, für alles
       andere stört es nicht. */
    ende: (e.end && e.end.date) || "",
    ganztags: !!start.date
  };
}

/* Ein Ferienblock: Titel enthält "Ferien", ganztägig, mehr als ein
   Tag. Ein einzelner Tag mit "Ferien" im Namen ist eher eine Notiz
   als ein Zeitraum. */
function alsFerien(t) {
  if (!t || !t.ganztags || !t.ende) return null;
  if (!/ferien/i.test(t.title)) return null;
  /* Googles Ende ist der Tag danach — einen zurück für den letzten
     Ferientag. Bewusst nicht über toISOString: das rechnet nach UTC
     und macht aus dem 30. den 29., solange Berlin vor UTC liegt. */
  const letzter = new Date(t.ende + "T00:00:00");
  letzter.setDate(letzter.getDate() - 1);
  const bis = letzter.getFullYear() + "-"
            + String(letzter.getMonth() + 1).padStart(2, "0") + "-"
            + String(letzter.getDate()).padStart(2, "0");
  if (bis <= t.date) return null;
  return { titel: t.title, von: t.date, bis };
}
...
async function ereignisseHolen() {
    ...
    try {
      ...
      (daten.items || [])
        ...
        .forEach(e => {
          ...
          if (!t) return;
          if (wirdAusgeblendet(t.title)) {
            ausgeblendet++;
            /* Aus der Terminliste raus, aber nicht aus dem Wissen:
               ein Ferienblock sagt, welche Tage keine Schultage sind. */
            const f = alsFerien(t);
            if (f && !ferien.some(x => x.von === f.von && x.bis === f.bis)) ferien.push(f);
            return;
          }
          alle.push(t);
        });
  ...
  return { termine: alle, ferien, gescheitert, ausgeblendet };
}
...
function alsEreignis(k, fachName, art, dauern) {
  ...
  /* Klausuren und Tests liegen in derselben Liste; das Merkmal am
     Eintrag entscheidet, wie das Ereignis sich nennt. */
  const wasIstEs = welche === "hausaufgabe" ? "Hausaufgabe"
                 : welche === "fahrschule" ? "Fahrschule"
                 : welche === "klausur" ? (k.pruefung === "test" ? "Test" : "Klausur")
                 : "Termin";
  ...
  return {
    ...
    ...zeitfelder,
    source: { title: "Life OS",
              url: "http://localhost:" + (global.lifeosPort || 3000) + "/#/lernen" },
...
/* Der Bestand ist die Ablage eines Nutzers — der Abgleich schreibt
   direkt dorthin, damit er auch ohne offenen Browser wirkt.

   Zu welchem Nutzer der Kalender gehört, steht beim Verbinden fest:
   der Google-Zugang ist ein persönlicher, und ein zweites Konto im
   Haus soll darüber weder lesen noch schreiben. Steht nichts da —
   etwa bei einer Verbindung von früher —, gilt das erste Konto. */
function nutzerFuerKalender() {
  const t = token() || {};
  if (t.nutzer) return t.nutzer;
  const alle = require("./nutzer").alleNutzer();
  return alle.length ? alle[0].id : null;
}

function bestand() {
  const id = nutzerFuerKalender();
  if (!id) throw new Error("Noch kein Konto eingerichtet");
  return require("./bestand").fuer(id);
}

/* Wem der Kalender gehört — oder null, solange keiner verbunden ist.
   Die Routen fragen damit ab, wer ihn benutzen darf. */
function kalenderNutzer() {
  return token() ? nutzerFuerKalender() : null;
}

/* Beim Verbinden festhalten, wem der Kalender gehört */
function kalenderNutzerSetzen(nutzerId) {
  const t = token();
  if (!t) return false;
  t.nutzer = nutzerId;
  schreiben(TOKEN, t);
...
async function abgleichLauf(fachNamen, dauern) {
  ...
  /* Ferien getrennt ablegen. Sie sind keine Termine — sie sind der
     Grund, warum „in 40 Tagen" und „in 40 Schultagen" verschiedene
     Dinge sind. */
  if (geholt.ferien && geholt.ferien.length) {
    const bisher = JSON.stringify(holenAusBestand("lifeos_ferien", []) || []);
    if (bisher !== JSON.stringify(geholt.ferien)) {
      bestand().setzen("lifeos_ferien", geholt.ferien, Date.now(), "google");
    }
  }
